...
最早的工作是由 Google 和 DeepMind 提出的两篇 Speculative Decoding，其模型结构和算法基本都一样，都是 Draft and Verify
伪代码如下：
...
EAGLE 之后的工作
- EGALE-2：优化了草稿树，将草稿树设置为动态草稿树来提高接受率
- EGALE-3：不只是使用倒数第二层特征，而是融合多层特征进行自回归
